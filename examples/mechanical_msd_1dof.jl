...
# 
# The harmonic oscillator is a classic example of a dynamic system characterized by its oscillatory behavior. In the mechanical domain, it can be represented as a mass-spring-damper system, as illustrated below:
# 
...
# ## Building the model
# 
# Prior to the definition of a system, we need to define the single port elements that will be connected 1-junction port (multi-port element). We define the mass-spring-damper element as follows:
...
# The equations of the model above were not simplified and represent the bond-graphs equations derived from 1-junction. Then, we simplify the DAE above and obtain the ODE. The simplified set of equations are required for using `solve` from `DifferentialEquations.jl`
