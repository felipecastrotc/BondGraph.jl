...
# For file history go to the fit-hist/dev_fit_indirect_md_modes.jl. 
# This is a cleaned file version of it, with some solved previous
...
function diffeq!(du, u, p, t)
    du[1] = (p[1] -  nn(u, p[3:end])[1]) / m̂!(u, p[2])
    du[2] = u[1]
end

#prob = ODEProblem(diffeqmsd!, [0.0, 0.0], (0.0, 20.0), [1e6, 1e6, 1e6, 1e6])
prob = ODEProblem(diffeq!, [0.0, 0.0], (0.0, 200.0), [])
...
# nn = FastChain(FastDense(7, 10, tanh), FastDense(10, 2))
# γ = initial_params(nn)
# mdl = Chain(Parallel(vcat, x -> x, x -> x.^2, x -> x.^3), Dense(9, 10, tanh), Dense(10, 2));
mdl = Chain(Parallel(vcat, x -> x, x -> x.^2, x -> x.^3), Dense(6, 10, tanh), Dense(10, 1));
γ, nn = Flux.destructure(mdl);
nn([1, 2], γ)
