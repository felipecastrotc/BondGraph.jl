...
F = 2
eqs =[D(ẋ1) ~ -k1/m1*x1 - c1/m1*ẋ1 + F/m1,
      D(x1) ~ ẋ1,
...
prob = ODEProblem(mdl, [0.0, 0.0, 0.0, 0.0], ts, Dict(k1 => 1.0, k2 => 20.0, c1 => 0.4, c2 => 0.5, m1 => 1.0, m2 => 1))
sol = solve(prob, reltol = 1e-8, abstol = 1e-8)
...
n = length(sol)
n = 1000
plot(sol.t[1:n], sum([u[i, :] for i in 2:2:5], dims=1)[1][1:n])
