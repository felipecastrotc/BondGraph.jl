...
if ModelingToolkit.isvariable(g)
    sts, ps = [], [g]
...
gy = compose(sys, pin, pout)
equations(gy)
# end manual mGY

@named psys = ODESystem([connect(je.power, gy.pin), connect(gy.pout, jm.power)], t)
mdl = compose(psys, gy, je, jm)
